...
// Storage adapter: saves to the browser's localStorage (per device).
// Mirrors the async API the app was originally written against.
const appStorage = {
  async get(key) {
...
export default function TruckEquipmentTracker() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await appStorage.get("equipment-entries");
        if (result && result.value) setEntries(JSON.parse(result.value));
      } catch (e) {}
      try {
        const cat = await appStorage.get("asset-catalogue");
  ...
  const persist = async (list) => {
    try {
      await appStorage.set("equipment-entries", JSON.stringify(list));
    } catch (e) {
  ...
  const importRegister = (file) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        setCatalogue(items);
        await appStorage.set("asset-catalogue", JSON.stringify(items));
